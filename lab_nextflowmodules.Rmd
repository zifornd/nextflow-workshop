...
nf-core tooling has commands for pipeline users and developers. To view all of the available commands use the nf-core --help argument.

```
nf-core --help
```

A new pipeline with a template of nf-core pipeline can be created using **nf-core pipelines create** command. Running this command will open a Text User Interface (TUI) for pipeline creation.
...
*Output*:
```{bash,comment=''}
tree -d bin/nf-modules/nf-core-testpipeline/
```
...
*Example:*
```{bash,comment=''}
cat bin/nf-modules/nf-modules/modules/cutadapt.nf
```
...
* If module is not already there then it can be created manually or a template can be created using the **nf-core modules create** command.
  ...
  *Example:*
  ```
  nf-core modules create tximport
  ```

  *Output*:
  ```{bash,comment=''}
  tree -d bin/nf-modules/nf-core-testpipeline/modules
  ```
...
* If subworkflow is not already there then it can be created manually or a template can be created using the **nf-core subworkflows create** command.
  ...
  *Output*:
  ```{bash,comment=''}
  tree -d bin/nf-modules/nf-core-testpipeline/subworkflows
  ```
...
*Example:*
```{bash,comment=''}
cat bin/nf-modules/nf-modules/subworkflows/bowtie2.nf
```
...
*Example:*
```{bash,comment=''}
cat bin/nf-modules/nf-modules/workflows/trimalign.nf
```
...
```{bash,comment=''}
cat bin/nf-modules/nf-modules/main.nf
```
...
```{bash,comment=''}
cat bin/nf-modules/nf-modules/nextflow.config
```
