...
*Example:*
```{bash,comment=''}
cat nf-testpipeline/modules/cutadapt.nf
```
...
* If module is not already there then it can be created manually or a template can be created using the **nf-core modules create** command.
  ...
  *Output*:
  ```{bash,comment=''}
  tree -d nf-testpipeline/modules
  ```
...
* If subworkflow is not already there then it can be created manually or a template can be created using the **nf-core subworkflows create** command.
  ...
  *Output*:
  ```{bash,comment=''}
  tree -d nf-testpipeline/subworkflows
  ```
...
*Example:*
```{bash,comment=''}
cat nf-testpipeline/subworkflows/bowtie2.nf
```
...
*Example:*
```{bash,comment=''}
cat nf-testpipeline/workflows/trimalign.nf
```
...
```{bash,comment=''}
cat nf-testpipeline/nextflow.config
```
